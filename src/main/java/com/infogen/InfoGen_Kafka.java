...
public class InfoGen_Kafka {
	...
	private static class InnerInstance {
		public static InfoGen_Kafka instance = new InfoGen_Kafka();
	}

	public static InfoGen_Kafka getInstance() {
		return InnerInstance.instance;
	}

	private InfoGen_Kafka() {
	}
	 ...
	 * @return
	 */
	public InfoGen_Kafka start(InfoGen_Configuration infogen_configuration) {
		if (producer == null) {
			// 设置配置属性
	...
	public void send(KeyedMessage<String, String> message) {
		if (producer != null) {
			try {
				producer.send(new KeyedMessage<String, String>(message.topic(), message.key(), IP.concat(",").concat(message.message())));
			} catch (Exception e) {
